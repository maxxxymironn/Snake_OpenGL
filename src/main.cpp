...
void draw(Renderer& renderer, const Game& game, const Clock& clock) {
	...
	if (updatedStaticData || (snakeMovingCoeff > 0.5f && head != lastHead)) {
		lastHead = head;
	...
	if (snakeMovingCoeff <= 0.5f) { 
		...
		if (newDirectionTail) {
			...
			diff = cur - next;
			if (diff.x != 0)
				pos.x += (diff.x == -1 || diff.x > 1) ? 4.f : -4.f;
			else
				pos.y += (diff.y == -1 || diff.y > 1) ? 4.f : -4.f;
	...
	if (drawTail) {
		if (tail != prevTail)
			...
			rotateAngle = getRotateAngle(tail, preTail);
